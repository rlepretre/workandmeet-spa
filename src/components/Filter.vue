        ...
        <q-btn rounded color="white" text-color="black" label="Tarif">
          <q-popup-proxy>
            <q-banner style="width: 600px">
              ...
              <q-btn
                rounded
                style="background: rgb(24, 22, 121); color: white"
                v-close-popup
                :priceModel="priceModel"
                @click="savePrice((itemClick = priceModel))"
                >Valider</q-btn
              >
            </q-banner>
          </q-popup-proxy>
        ...
        >
          <q-list v-for="espace in espacesType" :key="espace">
            <q-item
              clickable
              v-close-popup
              @click="placeItemFilter((itemClick = espace))"
            >
              <q-item-section>
  ...
  methods: {
    filter() {
      ...
      if (this.itemClick.count == 1) {
        this.filterd.push(this.itemClick.label);
        var filterArr = this.$store.state.first;
        var inputVal = this.filterd;
        this.advantageFilter(filterArr, inputVal);
        if (this.placeType != "") {
          var thirdArr = this.$store.state.everythings;
          var inputPlace = this.placeType;
          this.placeFilter(thirdArr, inputPlace);
        }
        if (this.price == "") {
          var thirdArr = this.$store.state.everythings;
          var inputPrice = this.price;
          this.priceFilter(thirdArr, inputPrice);
        }
      } else if (this.itemClick.count == 2) {
        this.itemClick.count = 0;
        var ind = this.filterd.indexOf(this.itemClick.label);
        this.filterd.splice(ind, 1);
        filterArr = this.$store.state.first;
        inputVal = this.filterd;
    ...
    advantageFilter(arr, val) {    //ADVANTAGE FILTER FUNCTION
