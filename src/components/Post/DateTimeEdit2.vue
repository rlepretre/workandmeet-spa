<template>
  <!-- DateTime Edit -->
    <div class="d-flex">
        ...
        <div class="d-flex">
            <div>{{ $store.state.searchInput.end }}</div>
            <div class="ms-2">
                <q-icon name="access_time" size="20px" class="cursor-pointer">                  
                <q-popup-proxy cover transition-show="scale" transition-hide="scale">
                    <q-time @click="calculate" v-model="$store.state.searchInput.end">
                      <div class="row items-center justify-end">
...
export default {
  ...
  data(){
    return {
      timeVal: this.$store.state.searchInput.start
    }
  },
  methods: {
    calculate(){
      if(this.$store.state.searchInput.date != '' && this.$store.state.searchInput.start != '' && this.$store.state.searchInput.end != ''){
        ...
        alert(this.$store.state.searchInput.diffTime)
      }
    },
  },
}
